...
**RESOLVED 2026-08-04:** replaced with real, rule-based analysis — deliberately no LLM, per the approved design decision in `docs/superpowers/specs/2026-08-04-ai-insights-design.md`. `getSystemAudit` now computes `healthScore` as 100 minus real deductions (pending schools, a bad teacher:learner ratio, and orphaned references) and only surfaces insights for conditions that actually fired, plus a single honest "No other issues detected" line when nothing did. `runDeepScan` calls a new generic dangling-reference detector, `backend/utils/orphanCheck.js`, across all 8 relationships that carry a subject/assignment/quiz reference (`Quiz`/`Assignment`/`StudyMaterial`/`ForumTopic`/`TeacherAllocation`/`LearnerEnrollment` → `Subject`; `Submission` → `Assignment`; `QuizAttempt` → `Quiz`), returning a real per-relationship breakdown with recommendations generated from actual findings instead of canned strings. The fabricated `lastBackup` date and `integrityScore` were dropped rather than faked differently — no application-code-computable equivalent exists (a real backup timestamp would need MongoDB Atlas API access this project doesn't have). The frontend's unconditional `alert('AI Deep Scan Complete. 0 critical security vulnerabilities found. 3 optimizations recommended.')`, which fired after every scan regardless of the real result, was also removed from `dev-ai-insights.js`. 12 new tests (`backend/test/orphanCheck.test.js`, `backend/test/aiController.test.js`) — backend now 86/86, frontend still 7/7.
