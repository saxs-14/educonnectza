...
`scripts/init-dev-admin.js`, `scripts/init-dev-admin.mjs`, and `backend/seed.js` all hardcode `mamagauphathu@gmail.com` / `Phathutshedzo@14` as the DevAdmin Firebase Auth credential. All three files are `git`-tracked (confirmed via `git ls-files`) and were part of the initial commit — the password is in git history permanently, regardless of any future edit to the files. This is a real personal Gmail-linked Firebase account credential, not a placeholder. **See the action box at the top of this report — this needs a password rotation by the project owner, not a code fix.** Once rotated, the fix in code is to have these scripts read the password from an environment variable / prompt, never hardcode it, and a `git filter-repo`/history-rewrite is worth considering separately if this repo is ever made public (out of scope for this report to execute).
